...
export default function OrderCard({ order, onViewDetails }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ['Docs', String(order.documents.length)],
      ...
      {/* Progress bar */}
      {stepIndex >= 0 && order.status !== 'rejected' && order.status !== 'cancelled' && (
        <div className="mb-4">
          <div className="flex justify-between mb-1.5">
            {STEPS.map((s, i) => (
              <span
                key={s}
                className={`text-[9px] font-bold capitalize ${i <= stepIndex ? 'text-[#FF6B00]' : 'text-gray-300'}`}
              >
                {s}
              </span>
            ))}
