  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      for (const item of items) {
        const documentIds = item.files.map((f: any) => f._id).filter(Boolean)

        await api.post('/orders', {
          documentIds:  documentIds.length ? documentIds : undefined,
          printOptions: item.options,
          deliveryType: 'pickup',
        })
      }
      clearCart()
  ...
  return (
    <div className="px-4 pt-6 pb-36 md:pb-10">
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[220px]">
                  {item.files.map((f: any) => f.name).join(', ')}
                </p>
              </div>
              <button onClick={() => removeItem(item.id)} className="p-2 hover:bg-red-50 rounded-xl transition-colors">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 md:static md:border-0 md:bg-transparent md:p-0">
        <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <Button size="lg" className="w-full" onClick={handleCheckout} disabled={loading}>
            {loading ? <><Loader className="w-4 h-4 animate-spin" /> Placing Order…</> : <>Place Order <ArrowRight className="w-4 h-4" /></>}
          </Button>
        </div>
      </div>
    </div>
